...
function App() {
  ...
  const runSosEmergency = async (resolvedTrusted: string): Promise<{ result: SosResult; mode: 'chain' | 'demo' }> => {
    ...
    <main className="shell">
      ...
      <section className="grid">
        ...
        <article className="card">
          ...
          <input type="file" onChange={(e) => onEvidenceUpload(e.target.files?.[0] ?? null)} />
          {evidenceError && (
          ...
          <div className="muted small">
            Upload proxies to Pin via your local `/api/ipfs/upload`. Start `PINATA_JWT=... npm run dev:api`.
          </div>
        </article>
